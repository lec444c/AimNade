...
AimNade 是面向 Counter-Strike 玩家、以本地内容驱动的战术道具学习 iOS App。通过分类列表、投掷方案详情与 2D 地图参考，帮助玩家查找和学习常用道具。

当前内置 **Mirage、Ancient、Nuke** 三张 2D 地图，支持 iPhone 与 iPad。所有内容随 App 打包，收藏与偏好保存在设备本地，无需联网或登录。

> 只有 Mirage 内置道具内容，而且仍是**未经实战核验的占位 / 示例数据**：共 7 个道具组、10 个投掷方案。其中新增的 T 方闪光弹、燃烧弹、手雷与 CT 方烟雾弹条目仅用于展示筛选界面，并已在内容中标明占位。30 张教学截图尚未提供。Ancient 与 Nuke 当前只提供地图预览，没有道具点位。

## 已实现功能

- **三栏主导航**：底部只保留战术、收藏、设置三个 Tab；启动后直接进入战术页，从顶部切换 Mirage / Ancient / Nuke。
- **三地图 2D 参考**：每张地图都支持平移、双指缩放和双击缩放；地图专注于清晰查看整体结构。Mirage 使用蓝橙主题，Ancient 使用绿色主题，Nuke 使用蓝色主题。
- **Mirage 列表优先**：进入有内容的地图时默认显示道具列表，按区域分组并直接显示“起点 → 目标点”；也可切换到纯净的地图参考视图。
- **按内容呈现控件**：Mirage 的搜索、道具类型和 T / CT 阵营状态由战术页统一管理；只有实际数据提供多个可选项时才显示对应筛选，避免单一选项占据首页空间。Ancient / Nuke 在暂无道具数据时直接显示地图预览。
...
当前占位数据覆盖 T / CT 阵营及烟雾弹、闪光弹、燃烧弹、手雷四种类型，用于展示首页筛选交互；这些条目不代表经过游戏内核验的教学内容。
...
当前采用轻量分层的 SwiftUI 架构。App 根视图为 `TabView`，战术、收藏、设置各自使用独立 `NavigationStack`。`TacticsView` 统一持有当前地图、地图/列表模式、阵营、道具类型和搜索状态。有道具数据的地图默认使用列表；`TacticalMapView` 仅负责显示和缩放当前地图。
...
- 道具组表示同一目标区域的一组投掷方案；每个方案记录起始区域、目标区域、投掷说明及三张教学图的资源名。
...
| [docs/LOCALIZATION.md](docs/LOCALIZATION.md) | 本地化约定、完整 81-key 清单、职责边界与审计方法 |
